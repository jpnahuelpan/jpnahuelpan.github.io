...
export default function Modal({ pdf, isOpen, closeModal }: Props) {
  ...
  return (
    <div id="pdfModal" className="fixed top-0 left-0 w-full h-full bg-stone-600 bg-opacity-50">
      <div className="group/parent group-hover/item:visible absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-5">
        ...
        <Document file={pdf} onLoadSuccess={onDocumentLoadSuccess} className={"border border-solid"}>
          <Page pageNumber={inPage} width={widthPdf}/>
          <div className="group/child flex justify-between items-center mx-auto w-36 my-8 shadow-stone-300 shadow-md rounded-lg transition duration-500 opacity-0 group-hover/parent:opacity-100">
